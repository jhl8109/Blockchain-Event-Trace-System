...
## 개요
허가된 블록체인은 공급망 관리, 의료, 투표 플랫폼 등 다양한 애플리케이션에 안전하고 투명한 솔루션을 제공할 수 있어 주목받고 있다. 그러나 블록체인 네트워크에 쌓이는 데이터의 양이 증가할수록 데이터에 대한 조회, 트랜잭션 수행 등의 성능 저하 문제가 발생한다. 이를 해결하기 위한 방안으로 본 논문에서는 이벤트 데이터를 시간적, 공간적, 참여자, 트랜잭션 데이터로 분류하여 오프체인 데이터를 형성하는 방안을 제시한다. 블록체인 실행환경에서 발생하는 이벤트를 수신 및 처리하는 메커니즘과 오프체인 데이터 분류를 제시함으로써 허가형 블록체인에서 오프체인 데이터를 구축하기 위한 기반 템플릿 모델로 본 방법이 적용될 수 있다. 사례연구로 Hyperledger Fabric에 구축된 P2P 중고품 거래 플랫폼을 제시하여 제안된 시스템이 활용 가능함을 보이고 오프체인 데이터 조회할 경우 회당 약 29ms 정도의 성능이 개선됨을 확인하였다.

## 구성

![스크린샷 2023-05-23 오전 12 06 16](https://github.com/jhl8109/Restful-Service/assets/78259314/5e9be185-7cca-41cc-9646-c7762b80dfce)
